...
# -*- coding: utf-8 -*-
"""
o0_temporal_probe_v4.py  --  CONCEPT temporal channel V5 probe (live Wikidata/Wikipedia).

V5 vs V4 (REPORT_temporal_probe_v3.md sec 3/5): closes D1'' -- the CONFIRM co-location +-WINDOW=200
CHAR span bled ACROSS sentences, so TRAP_unconfirm spuriously CONFIRMed 1968 (a 2024-edition
"published" sat within +-200 of an unrelated filmography "1968"). CONFIRM was UNSOUND. Single
surgical change in confirm_in_text():
    SCOPE: +-WINDOW chars  ->  SAME-SENTENCE. CONFIRM iff the year AND an admissible-class event
    verb appear in ONE article sentence. Predicate-class anchor + "== header ==" strip + LEFT
    word-boundary (all V4) RETAINED.
  Same-sentence is monotonically tighter than +-WINDOW: removes confirms, never adds. So 18 stays
  ABSTAIN and no trap gains a confirm. TRAP_unconfirm 1968 -> ABSTAIN (1968 filmography line has no
  in-sentence release verb) -> finally exercises refute-restraint (R15 / D4), instead of the CONFIRM
  short-circuit.

  HARD REGRESSION GUARD (the V4-named watch item, now load-bearing): 16 + 09 MUST stay CONFIRM
  same-sentence.
    16 (1847): "In 1847, he proposed..." -> "propos" (P575/P585, claim class) in-sentence -> HOLDS
               (verified offline, see SELFTEST).
    09 (1887): "...November 1887 with his paper..." -- "paper"=P577 is OFF-CLASS for claim_09
               {P575,P585}; the admissible "observing" was the PRIOR sentence under +-200. 09 holds
               iff the FULL 1887 sentence (beyond the old +-200 window) carries a P575/P585 verb.
               UNRESOLVABLE from the truncated v3 snippet -> LIVE regression risk. If 09 ABSTAINs
               live, same-sentence is too strict and P1 FAILs by design (the finding), NOT a tuning
               license: report it, escalate to V6 (sentence + governing-clause), do NOT loosen here.

  D2 (person-dominant resolve) + D3 (person-name refute block + event-predicate match): FROZEN.
  EVENT_PREDICATES, refute path, WD-predicate confirm path: UNTOUCHED.
...
FROZEN BARS (pre-registered -- do NOT tune after results):
  P1 (CONFIRM, HARD regression guard): heldout_16 (1847) AND heldout_09 (1887) BOTH CONFIRM
                      under SAME-SENTENCE. heldout_14 stays N/A (1900 in abstract).
  ...
  P3 (REFUTE-restraint, HARD, load-bearing): TRAP_python, TRAP_titanic, TRAP_norefute,
                      ...
                      ABSTAIN = R15 CLOSED (refute-restraint now actually testable) /
                      CONFIRM = D1'' NOT closed (bleed remains) / REFUTE = D4 work-decoy gap.
PASS = P1 AND P2 AND P3.
Falsifiers: F-D1'' (TRAP_unconfirm CONFIRM = same-sentence did not close the bleed);
  F-regress (16 or 09 ABSTAIN = same-sentence too strict); F-D3 (03 anchor != Q24563361);
...
def main():
    ...
    print("\n=== TEMPORAL CHANNEL V5 PROBE -- claim verdicts ===")
    ...
    print("R15 sub-check TRAP_unconfirm pass-mode  :", unconfirm_mode,
          "(ABSTAIN=R15 closed/refute testable ; CONFIRM=D1'' NOT closed ; REFUTE=D4 work-decoy gap)")
    overall = "PASS" if (p1 and p2 and p3) else "FAIL"

    print("\nFALSIFIERS:")
    print("  F-D1''   (TRAP_unconfirm CONFIRM)   :", "TRIGGERED" if cv("TRAP_unconfirm") == "CONFIRM" else "clear")
    ...
    json.dump(trace, open("o0_temporal_probe_v4_trace.json", "w"), indent=2)
    print("\ntrace -> o0_temporal_probe_v4_trace.json")

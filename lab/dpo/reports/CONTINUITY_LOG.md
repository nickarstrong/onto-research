...
v300 CONCEPT: BARE-QUANTITY WD numeric-property eligibility SPEC sealed (GENGAP_barequant_v300.md, md5_8 0ea0bfe6). PICK=bare-quantity (granularity UNEXERCISED, rejected). Bars pre-registered B0/B-numprop-cov>=0.67/B-numdiscrim/B-noregress/B-hermetic. Design GREEN, eligibility OPEN. HEAD 126390f. Founder-gated: TENSION-1 v4-additive-compat, TENSION-2 v272 oracle-seam precondition; public extraction OWED.
v301 numprop eligibility PROBE: RED. cov 0.000 < 0.670 (bar held). Cause = SUBJECT-RESOLUTION floor (bare common-noun subjects share en-label across many WD senses), NOT numeric-property absence. Instrument validated across 3 read-only iters; circular property-conditioned resolve rejected. DEFERRED -> v302 CONCEPT: QID-anchored property-floor re-pose (A) / disambiguator floor (B) / granularity-guard (C); Founder-gated with TENSION-1/2. Diag sealed LOCAL GENGAP_barequant_v301_diag.md.
v302 numprop eligibility DECOMPOSITION (CONCEPT): split conflated eligibility into RESOLUTION floor (i, property-INDEPENDENT bind) + PROPERTY floor (ii, given-QID numeric P with convertible V); property floor is the sole UNMEASURED leg (v301 died on resolution). Option A pre-registered: QID-anchored property-floor probe, bar B-numprop-cov-given-resolution>=0.67, N>=15 QIDs / >=3 property-classes, circularity structurally barred (QID is the anchor, no resolve step). Founder rulings: TENSION-1 v4-additive-compat=CLEARED; TENSION-2 v272-oracle-seam=NOT-BIND for Option A (re-check at Option B); A/B/C route=A-first, B iff property floor passes, C if it fails. Sealed LOCAL GENGAP_numprop_repose_v302.md md5_8 7233227a. HEAD 126390f. No code/commit/measurement this plane.
v303 numprop PROPERTY-FLOOR PROBE (TYPE-A): QID-anchored property-floor harness (read-only, reuses v301c parse, time-units added for P2047, search/bind DROPPED, QID=anchor -> circularity structurally barred). N=17 QIDs / 6 property-classes (population/elevation/height/duration/area/mass). Construct-validity guard (label_en echo vs subject) FIRED on run1: 2 area QIDs wrong (Q5512->'1263', Q5511->Lake Tanganyika); corrected to Q5513/Q1066, re-run. cov_given_resolution=1.000 (17/17) vs bar B-numprop-cov-given-resolution>=0.67 -> PASS. B-hermetic: cache byte-identical across 2 cold runs md5_8 1167dcc4. Mechanism: given resolution, WD positively carries expected numeric-P with convertible unit for ~all salient post-resolution entities -> PROPERTY floor NOT the numprop bottleneck; RESOLUTION is (v301 cov 0.000). R3 caveat: cov=1.000 = zero discriminative-failure surface, verdict conditional on resolution filter (which selects salient). Route per A/B/C (v302): PASS -> Option B (resolution-disambiguator floor) next plane; Option C not needed. Probe/harness/cache/report KEEP-LOCAL. HEAD 126390f, no commit (Founder-gated), no frozen-seam edit, no verdict-pool mutation. Report propfloor_v303_report.md md5_8 8771721e.

v304 CONCEPT: Option B resolution-disambiguator design sealed (GENGAP_optionB_resoldisambig_v304.md md5_8=0fd9f86e). Floor=P31 type-class (P279* closure K=8) + sitelink salience (Delta>=2x dominance), property-INDEPENDENT, ABSTAIN-preserving. Bar pre-registered: B0 absolute 0 false-bind + B-resoldisambig-cov>=0.70 resolvable + B-hermetic. TENSION-2=NOT-BIND (oracle-side read-only, v299/v301c/v303 lineage, upstream of verdict gate; re-opens only if wired into verdict path). Route -> v305 TYPE-A build.v305 TYPE-A OptionB resol-disambig BUILD+MEASURE: GREEN. B0 PASS(0 false-binds); B-resoldisambig-cov 0.833(10/12, 2 ABSTAIN-miss=wbsearch surface-gap, river-GT not in top5); B-hermetic PASS(report md5_8 cd3f5c70). harness 26119238 probe b4a725d4 cache+report KEEP-LOCAL. floor P31+P279*(K8)+sitelink(d2x) validated; TENSION-2 NOT-BIND held (oracle-side ro). caveat: 2/4 traps rule5 co-equal(Ouse,Newport), 2/4 rule3 no-type(Avon,Stour), all ABSTAIN B0 holds. no commit (Founder-gated).
v306 CONCEPT (wire-in + C-2 unblock): GENGAP_optionB_wirein_v306.md sealed 6aa8ec68 (KEEP-LOCAL/111). TENSION-2 RE-RULED = NOT-CROSS conditional: numprop wire-in -> numeric no-op leg only (year-leg disjoint); seam-safe = new module o0_layer2_numprop.py + new composed driver, ZERO frozen-anchor edit (post-build md5 re-check authorises v308). New risk: live expected_class via P->domain-class (property-identity not value) -> B-domain sub-bar. v308 bar FIXED: B0 + B-domain + B-cov>=0.50 on NEW live-form held-out + B-noregress + B-hermetic. C-2 DISPOSITION = INDEPENDENT (honest ABSTAIN floor before wired measure), own TYPE-C Founder-gated GT-correction. Route v307 TYPE-C C-2 -> v308 TYPE-C wired build. HEAD 126390f.
v308 TYPE-C wired numprop BUILD: 3 additive files committed (o0_layer2_numprop.py 926d10a8 + o0_layer2_driver_v308.py de7998ba + p_domain_map_v308.json 335a59cb). ALL BARS GREEN: B0 PASS(0 false-DIRTY on 8 GT-CLEAN), B-domain PASS, B-cov 0.625>=0.50 (5/8 GT-DIRTY REFUTE), B-noregress PASS(year-leg all-ABSTAIN), B-hermetic PASS(edbee196). TENSION-2 NOT-CROSS confirmed (6/6 frozen anchors byte-identical). rank-aware read_wd_prop fix mid-build (P1082 multi-valued preferred-rank, Rome false-DIRTY root cause). HEAD 126390f->8288a0e. Held-out/verdicts/cache KEEP-LOCAL.

## v309 (TYPE-B - wired-driver impact measurement on main held-out)
- Ran v308 composed driver on main held-out (48 rows). Gate ALL-PASS: B0=0 false-DIRTY/35-clean, B-event-anchor + B3-WATCHF + B-discrim PASS.
- Year-leg BYTE-IDENTICAL to frozen baseline (0 mismatch); 0 verdict flips -> wire-in non-destructive, zero regression.
- Numprop-leg impact = 0 (0 flips, 0 deferred-S-class catch): 4 rows no_mapped_property (P-domain map coverage gap), 1 row property-absent-on-bound-entity (candidate mis-map, WATCH).
- Conclusion: wire-in SAFE + INERT on this substrate; numprop coverage gap is the bottleneck.
- Route -> v310 CONCEPT (numprop coverage: extend P-domain map vs formal defer ruling). Measurement verdict file KEEP-LOCAL; no code commit.

...
W_GRID = sorted(set([round(0.25 * i, 2) for i in range(0, 5)]))   # 0.00, 0.25, 0.50, 0.75, 1.00 (coarse; W=0 = E35 lexical regression anchor)
...
def item_label_at_C(resolutions, C, B_floor=0.0, scope=PRIMARY_SCOPE, A=float("inf"), W=0.0):
    """E35 three-way (ACCEPT/REJECT/ABSTAIN) over the bound subset, layered on the FROZEN E34b reject gate.
    Per bound candidate (B>=B_floor): REJECT iff contradiction>=C ; ACCEPT iff entailment>=A.
    A candidate's reject is RESCUED at the CLAIM level iff some bound candidate (same scope) ACCEPTs ->
    effective veto = (a bound candidate rejects) AND (no bound candidate accepts). The reject predicate
    (con>=C AND B>=B_floor) is UNCHANGED from E34b; the accept arm only rescues.
    DEGENERACY: A=+inf => ent>=A never true => accept never fires => effective veto == E34b reject -> the
    three-way collapses to the EXACT E34b binary (regression anchor; A10).
    `scope` selects WHICH candidate(s) participate (reject AND accept use the same scope filter):
       'any'             : any bound candidate (E35 PRIMARY).
    ...
    Baseline C=+inf => no candidate rejects under ANY scope/B_floor/A => only noauth demotes => no-veto fa 0.467
    (scope/B_floor/A-independent, model-independent). Rollup identical to run_E26/E27/E28 (UNVERIFIABLE -> DEMOTE).
    r[1] top-1-cosine-first (asserted in main); r[1][0] = bound support. Tuples = (cos, contradiction, entailment, B_lex, B_anchor).
    E36: effective binding b = (1-W)*B_lex + W*B_anchor ; W=0 => b == B_lex byte-identical => EXACT E35 path."""
    labs = []
    for r in resolutions:
        if r == "common":   labs.append("PASS-COMMON")
            ...
            if scope == "top1":
                cos0, sc0, en0, bl0, ba0 = cands[0]
                b0 = (1.0 - W) * bl0 + W * ba0                # E36: effective binding (W=0 => == bl0 byte-identical)
                reject = (sc0 >= C and b0 >= B_floor)
                accept = (en0 >= A and b0 >= B_floor)
            elif scope == "any":
                reject = any((sc >= C and ((1.0 - W) * bl + W * ba) >= B_floor) for (cos, sc, en, bl, ba) in cands)
                accept = any((en >= A and ((1.0 - W) * bl + W * ba) >= B_floor) for (cos, sc, en, bl, ba) in cands)
            elif isinstance(scope, tuple) and scope[0] == "margin":
                delta = float(scope[1]); cos0 = cands[0][0]
                reject = any((sc >= C and ((1.0 - W) * bl + W * ba) >= B_floor and cos >= cos0 - delta) for (cos, sc, en, bl, ba) in cands)
                accept = any((en >= A and ((1.0 - W) * bl + W * ba) >= B_floor and cos >= cos0 - delta) for (cos, sc, en, bl, ba) in cands)
            else:
                raise ValueError(f"unknown scope {scope!r}")
            veto = reject and not accept                   # E35 accept-protective rescue (A=+inf => accept False => == E34b)
...
def metrics_at_C(items_pre, C, B_floor=0.0, scope=PRIMARY_SCOPE, A=float("inf"), W=0.0):
    res = [(it, item_label_at_C(r, C, B_floor, scope, A, W)) for (it, r) in items_pre]
...
def s2_band_at_bfloor(items_pre, B_floor, scope=PRIMARY_SCOPE, A=float("inf"), W=0.0):
    ...
    E35: read at the op-point's selected accept threshold A (A=+inf reproduces the E34b band)."""
    rows = []
    for C in S2_GRID:
        B1, B2, _B3, _ = metrics_at_C(items_pre, float(C), B_floor, scope, A, W)
...
def s2_perBfloor_summary(items_pre, scope=PRIMARY_SCOPE, A=float("inf"), W=0.0):
    ...
    E35: evaluated at the op-point's accept threshold A (A=+inf reproduces the E34b summary)."""
    out = []
    for B_floor in B_FLOOR_GRID:
        rows, band = s2_band_at_bfloor(items_pre, float(B_floor), scope, A, W)
...
def s3_attribution(items_pre, op_C, op_B_floor, fa_base, fa_op, scope=PRIMARY_SCOPE, A=float("inf"), W=0.0):
    ...
    for r in spoofs:
        ...
        op = item_label_at_C(r, float(op_C), float(op_B_floor), scope, A, W)      # three-way at op (accept may rescue)
        if op == "DEMOTE":
            n_demote_op += 1
    ...
    for r in golds:
        rej_only = item_label_at_C(r, float(op_C), float(op_B_floor), scope, float("inf"), W)
        with_acc = item_label_at_C(r, float(op_C), float(op_B_floor), scope, A, W)
...
def main():
    ...
    ap.add_argument("--out", default="reports/report_E36.md")   # NEVER a frozen-report path (no clobber E35/E34b/E32/E30/E28/E27)
    a = ap.parse_args()
    ...
    A_sweep = list(A_GRID)
    W_sweep = list(W_GRID)   # E36 anchor-mix axis (4th selection axis; W=0 = lexical regression anchor)

    # ---- op-point selector (FROZEN selection; E35 adds the accept axis A) ------------------------------------
    # over (A_GRID x OBSERVED B_floor x OBSERVED C): MIN false-accept subject to B2>=0.90. Tie-break (frozen):
    # widest joint C-band at that (A,B_floor) -> lowest C -> highest B_floor -> HIGHEST A (safest accept: fewest
    # spoof rescues among equal-fa points). selection peeks ONLY recall feasibility (B2>=0.90), never fa, never A
    # per-class -> oracle-clean (sec4). A=+inf is NOT in the sweep (it is the regression anchor below).
    def select_op(scope):
        # E36: 4-axis (W x A x B_floor x C). MIN fa s.t. B2>=0.90. Tie-break BYTE-IDENTICAL to E35
        # (widest C-band -> lowest C -> highest B_floor -> highest A) + LOWEST-W appended last: the anchor
        # (W>0) is chosen ONLY if it strictly lowers min-fa or widens the C-band; W=0 wins all ties ->
        # exact E35 op reproduced at the degenerate point. Peeks ONLY recall (B2>=0.90); oracle-clean.
        feasible = []
        for W in W_sweep:
            for A in A_sweep:
                for Bf in B_obs:
                    for C in C_obs:
                        b1, b2, _b3, _ = metrics_at_C(items_pre, C, Bf, scope, float(A), float(W))
                        if not np.isnan(b2) and b2 >= RECALL_FLOOR:
                            feasible.append((1.0 - b1, float(C), float(Bf), float(A), float(W)))
        if not feasible:
            return None, None, None, None, None, float("nan")
        min_fa = min(f for f, _c, _bf, _a, _w in feasible)
        cand = [(c, bf, a, w) for (f, c, bf, a, w) in feasible if abs(f - min_fa) < 1e-12]
        _bw = {(bf, a, w): s2_band_at_bfloor(items_pre, bf, scope, a, w)[1]["width"] for (_c, bf, a, w) in cand}
        cand.sort(key=lambda t: (-_bw[(t[1], t[2], t[3])], t[0], -t[1], -t[2], t[3]))
        c_, bf_, a_, w_ = cand[0]
        _b1, b2_, _b3, _ = metrics_at_C(items_pre, c_, bf_, scope, a_, w_)
        return float(a_), float(c_), float(bf_), float(w_), float(b2_), float(min_fa)
    ...
    anc_A, anc_C, anc_Bf, anc_W, anc_B2, anc_fa = select_op(ANCHOR_SCOPE)   # ANCHOR_SCOPE == PRIMARY_SCOPE == 'any'
    anchor_below_bar = (anc_C is not None) and (not np.isnan(anc_fa)) and (anc_fa < E28_OP_FA)
    print(f"[anchor-compare scope=any] A={anc_A} C={anc_C} B_floor={anc_Bf} W={anc_W} fa_op={anc_fa} "
          f"vs bart bar {E28_OP_FA} / deberta bar {DEBERTA_ANCHOR_FA} -> below_bart={anchor_below_bar}")

    # E35 PRIMARY op-point: scope='any' over the BOUND subset (B>=B_floor), accept threshold A from the sweep.
    chosen_A, chosen_C, chosen_Bf, chosen_W, chosen_B2, _min_fa = select_op(PRIMARY_SCOPE)
    if chosen_C is None:
        op_verdict = "FAIL_RECALL_UNUSABLE"; B1 = B2 = B3 = fa = move = float("nan")
    else:
        B1, B2, B3, _ = metrics_at_C(items_pre, chosen_C, chosen_Bf, PRIMARY_SCOPE, chosen_A, chosen_W)
    ...
    W_for_band = chosen_W if chosen_W is not None else 0.0
    s2_rows, band = s2_band_at_bfloor(items_pre, float(Bf_for_band), PRIMARY_SCOPE, A_for_band, W_for_band)
    s2_perBf = s2_perBfloor_summary(items_pre, PRIMARY_SCOPE, A_for_band, W_for_band)
    bf_clearing = [row for row in s2_perBf if row[6]]          # B_floor points whose min-fa clears GATE_FA (at chosen_W)
    bf_band_points = len(bf_clearing)
    bf_band_nontrivial = bf_band_points >= S2_BAND_MIN_POINTS  # sec6: not a single-point knife-edge
    # E36 per-W bf-band DIAGNOSTIC (reported-only; feeds the KNIFE_EDGE-vs-BINDING_EXHAUSTED falsifier fork,
    # NEVER the op-selection). For each W: count B_floor points clearing the gate (at chosen_A). best-over-W
    # nontrivial => some anchor weight lifts the bf-band even if the conservative op-selector did not pick it.
    bf_by_W = []
    for W in W_sweep:
        _perBf_W = s2_perBfloor_summary(items_pre, PRIMARY_SCOPE, A_for_band, float(W))
        bf_by_W.append((float(W), sum(1 for row in _perBf_W if row[6])))
    bf_band_best_W = max((c for _w, c in bf_by_W), default=0)
    bf_band_best_nontrivial = bf_band_best_W >= S2_BAND_MIN_POINTS
    if chosen_C is not None and base_ok:
        s3 = s3_attribution(items_pre, chosen_C, chosen_Bf, fa_base, fa, PRIMARY_SCOPE, chosen_A, chosen_W)
    else:
    ...
    for delta in MARGIN_GRID:
        mA, mC, mBf, mW, mB2, mfa = select_op(("margin", float(delta)))
        if mC is None:
            margin_rows.append((delta, None, None, float("nan"), float("nan"), False, float("nan")))
        else:
            _r, mband = s2_band_at_bfloor(items_pre, float(mBf), ("margin", float(delta)), mA, mW)
            ms3 = s3_attribution(items_pre, mC, mBf, fa_base, mfa, ("margin", float(delta)), mA, mW) if base_ok else \
                  dict(reject_share=float("nan"))
            margin_rows.append((delta, mC, mBf, mfa, mB2, mband["robust"], ms3["reject_share"]))

    # ---- E35 verdict (PRE_REGISTER_E35.md sec5-6; reads scope=any fa_op vs GATE 0.10 + reject-only 0.1667 + bf-band) -
    # VOID guards INHERIT E34b: baseline mismatch / degenerate rho / B_floor=0 degeneracy (0.2333) / recall-unusable.
    # E35 ADDS: accept-off regression (A=+inf MUST reproduce E34b reject-only 0.1667) -> else VOID_ACCEPT_REGRESSION.
    # The forks read fa_op (primary scope=any, accept on); TWO_ORGAN_LEVER additionally requires a NON-TRIVIAL bf-band.
    # n_spoof_rescued (accept leak) reported; a TWO_ORGAN_LEVER that clears the gate ONLY via spoof rescues is caught
    # by fa itself (rescued spoofs ARE false-accepts -> raise fa) -- the metric is leak-honest by construction.
    ...
    if not base_ok:                 verdict = "VOID_BASELINE_MISMATCH"   # noauth path broke (model-independent)
    ...
    elif not degen_ok:              verdict = "VOID_DEGENERACY"          # B_floor=0,A=+inf,W=0 did NOT reproduce 0.2333
    elif not rejonly_ok:            verdict = "VOID_ACCEPT_REGRESSION"   # A=+inf,W=0 did NOT reproduce E34b reject-only 0.1667
    elif not bindreg_ok:            verdict = "VOID_BINDING_REGRESSION"  # W=0 did NOT reproduce E35 TWO_ORGAN op 0.0667
                                    ...
                                    verdict = "TWO_ORGAN_LEVER"          # anchored binding clears fa<=0.10, B2>=0.90, NON-TRIVIAL bf-band
    elif (fa <= GATE_FA) and bf_band_best_nontrivial:
                                    verdict = "TWO_ORGAN_KNIFE_EDGE"     # clears gate; chosen-W bf-band trivial BUT some W lifts it -> tighten/rerun
    elif fa <= GATE_FA:             verdict = "BINDING_EXHAUSTED"        # THE FALSIFIER: clears gate but NO W lifts the bf-band -> per-candidate binding EXHAUSTED -> pivot to cross-source CONSISTENCY over the SET
    elif fa < E34B_REJECT_OP_FA:    verdict = "ANCHOR_BINDING_HELPS_PARTIAL"  # 0.10 < fa < 0.1667 (cannot occur if W=0 in sweep; kept for completeness)
    else:                           verdict = "BINDING_EXHAUSTED"        # fa >= 0.1667 (cannot occur: E36 is a superset of E35) -> falsifier
    ...
    # ---- report --------------------------------------------------------------------------------------------
    _prereg_path = os.path.join(os.path.dirname(a.out), "PRE_REGISTER_E36.md")
    _prereg_md5 = md5(_prereg_path) if os.path.exists(_prereg_path) else "NOT-ON-DISK"
    L = [
        "# report_E36 -- anchored-binding REPLACE (B=(1-W)*lexical+W*anchor) on the FROZEN E35 three-way ; W=0 regression == E35 ; vs E35 op 0.0667 + bf-band (S1/S2/S3)",
        "",
        f"- pre-register   : PRE_REGISTER_E36.md (FROZEN md5 {_prereg_md5}) -- sec1/2/4/5/6 operative ; three-way reject/accept inherits PRE_REGISTER_E35 (8c719b0bdc88250165bce434c9652eea)",
        f"- lever          : THREE-WAY (FROZEN predicate) -- REJECT (con>=C AND B>=B_floor) RESCUED by ACCEPT (ent>=A AND B>=B_floor); B SHARPENED: {ANCHOR_NOTE} ; W swept over W_GRID; premise=source.FINDING / hyp=claim ; MODEL={CE_MODEL} (UNCHANGED)",
        ...
        f"- binding axis   : W swept over FROZEN W_GRID {W_GRID} ; B=(1-W)*B_lexical+W*B_anchor ; W=0 = lexical regression anchor (== E35, op MUST reproduce {E35_OP_FA})",
        ...
        f"- chosen A (accept) = {chosen_A} | chosen C = {chosen_C} | chosen B_floor = {chosen_Bf} | chosen W (anchor-mix) = {chosen_W} | B2 at op-point = {chosen_B2}",
    ...
    L += [
        ...
        f"- bf-band non-triviality (verdict input, at chosen W={chosen_W}): {bf_band_points} B_floor points clear GATE_FA -> nontrivial={bf_band_nontrivial} (bar >= {S2_BAND_MIN_POINTS})",
        f"- bf-band best-over-W (DIAGNOSTIC, feeds KNIFE_EDGE-vs-BINDING_EXHAUSTED fork): max clears={bf_band_best_W} -> best_nontrivial={bf_band_best_nontrivial}",
        "- per-W bf-band clearing (W, B_floor-points clearing gate at chosen A) [DIAGNOSTIC; NOT selection]:",
        *[f"    W={w:.2f}  clears={c}" for (w, c) in bf_by_W],
    ...
    L += [
        ...
        f"## VERDICT : {verdict}",
        "",
        "decision (PRE_REGISTER_E36.md sec6, operative forks -- read scope=any fa_op (accept ON, W swept) vs gate 0.10 + bf-band@chosen-W + best-over-W):",
        "  TWO_ORGAN_LEVER (fa_op <= 0.10 AND B2>=0.90 AND bf-band@chosen-W nontrivial) -> anchored binding lifts the bf-band off the floor",
        "    the gate -> proceed to robustness sweep + verifier-integration (rho 0.95-vs-0.50 reconcile, full-GOLD scale). TOMMY GO REQUIRED.",
        "  TWO_ORGAN_KNIFE_EDGE (fa_op<=0.10, bf-band@chosen-W trivial, but SOME W lifts it best-over-W>=3) -> tighten/widen, latent lever ->",
        "    directionally right, insufficient -> a third signal or a different binding observable (design, next).",
        "  BINDING_EXHAUSTED [FALSIFIER] (fa_op<=0.10 but NO W lifts the bf-band) -> per-candidate binding EXHAUSTED -> pivot to cross-source",
        "    organ class is DONE -> pivot to a structurally different verifier (cross-source consistency over the candidate SET).",
        "  VOID_* -> trust gate failed (baseline / rho / B_floor=0!=0.2333 / A=+inf!=0.1667 reject-only / W=0!=0.0667 binding-regression / recall) -> reconcile, re-run.",
        "",

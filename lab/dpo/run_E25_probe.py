...
# E25 = E24 + DIAGNOSTICS readout (no new data, no new model, SAME gate; verdict logic byte-stable from E24).
#   E24 showed content premise (37%) drops false-accept 0.333->0.20 but T stays ~0. Open question (pre-registered,
#   PRE_REGISTER_E25.md): is the gain real GOLD-content separation, or only spoof-REJECTION (gold entailment also
#   pinned low)? E25 emits per-(item-class, premise-kind) entailment distributions + content-premise separation at
#   chosen_T. CORPUS GO-GATE (cheap, before any full-content authoring): gold-content p50 clearly ABOVE spoof-content
#   p50 -> full-content bind-corpus JUSTIFIED (build E26). gold-content ALSO pinned ~0 -> corpus alone insufficient
#   -> pivot passage-NLI/contradiction. This is the cheapest discriminator BEFORE the expensive corpus build.
...
def main():
    ...
    ap.add_argument("--out", default="reports/report_E25.md")
    a = ap.parse_args()
    ...
    # --- E25 diagnostics (additive; verdict above unchanged) ------------------------------
    def _summ(vals):
    ...
    L += ["", "## E25 diagnostics -- entailment score by (item-class, premise-kind)"]
    for key in sorted(groups): L.append(f"- {key[0]:28s} premise={key[1]:7s} : {_summ(groups[key])}")
    gc  = [sc for cls, k, sc in diag if k == "finding" and cls == B2_CLASS]   # gold, content
    spc = [sc for cls, k, sc in diag if k == "finding" and cls == B1_CLASS]   # spoof, content
    gc_p50  = float(np.percentile(gc, 50)) if gc else float("nan")
    spc_p50 = float(np.percentile(spc, 50)) if spc else float("nan")
    sep = gc_p50 - spc_p50
    T = chosen_T if chosen_T is not None else float("nan")
    ga = sum(s >= T for s in gc) if (gc and chosen_T is not None) else float("nan")
    sa = sum(s >= T for s in spc) if (spc and chosen_T is not None) else float("nan")
    pinned = "~0 (pinned)" if (chosen_T is not None and chosen_T < 0.01) else "lifted"
    L += [
        "",
        f"## content-premise separation (chosen_T={T:.5f}, {pinned})",
        f"- gold-content  : {_summ(gc)} | >=T: {ga}/{len(gc)}",
        f"- spoof-content : {_summ(spc)} | >=T: {sa}/{len(spc)}",
        f"- gold-vs-spoof content p50 separation = {sep:.4f} (gold {gc_p50:.4f} - spoof {spc_p50:.4f})",
        "",

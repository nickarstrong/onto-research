...
$N = (Get-Content "$dpo\data\onto_e$($E)_dpo_pairs.jsonl" | Measure-Object -Line).Lines
$md5 = (Get-FileHash "$dpo\data\onto_e$($E)_dpo_pairs.jsonl" -Algorithm MD5).Hash.ToLower()
$sh = "#!/bin/bash`nset -e`ncd /content/working`necho '=== MD5 CHECK ==='`nmd5sum data/onto_e$($E)_dpo_pairs.jsonl`necho 'expect: $md5 (mismatch = STOP)'`nunzip -o $Adapter.zip -d /content/working`necho '=== TRAIN ==='`npython onto_dpo_train.py --data data/onto_e$($E)_dpo_pairs.jsonl --adapter $Adapter --epochs $epochs --lr $lr --beta $beta --output adapter_E$($E)_dpo_$N`necho '=== PACK ==='`nzip -rq adapter_E$($E)_dpo_$N.zip adapter_E$($E)_dpo_$N`ncp adapter_E$($E)_dpo_$N.zip /workspace/`necho 'DONE: /workspace/adapter_E$($E)_dpo_$N.zip - Download NOW'"

# %% [markdown]
# # ONTO В· Experiment #1 В· E7 вЂ” SFT-only Reflex LoRA + targeted guard density (Qwen2.5-Coder-7B base)
#
# **Scope (E7 run):** RECIPE = E2 VERBATIM (lr1e-4, all-linear) вЂ” UNCHANGED from E3/E4. ONLY data changed:
# `sft_reflex_418.jsonl` (323 + 69 E7 guard pairs: 40 P6_premise_guard + 17 P6_disclaim_guard + 12 P6_anchor),
...
# re-written + downloaded after EACH arm вЂ” a crash mid-inference costs at most one arm, not the run.
#
...
#   - sft_reflex_418.jsonl   (418 rows: instruction + output + category; incl. 59 P5_refusal_on_unknown)
...
# %%
# ---- HARD ORDER: datasets BEFORE torch (01_LAB В§3) ----
import os, sys, subprocess

def pip(*pkgs):
    subprocess.run([sys.executable, "-m", "pip", "install", "-q", *pkgs], check=True)

# Kaggle T4 image is usually current enough; pin only what matters. No unsloth, no trl (01_LAB В§3).
pip("-U", "peft", "bitsandbytes>=0.46.1")

import datasets            # noqa: F401  (import BEFORE torch вЂ” reverse = silent crash, 01_LAB В§3)
...
# %%
# ============================ CONFIG (HARD, 01_LAB В§4 + README_S1) ============================
SEED            = 42
...
def find_input(fname):
    ...
    for root in roots:
        if root.exists():
            ...
            if hits:
                return hits[0]
    raise AssertionError(f"STOP: {fname} not found вЂ” upload it (Colab: Files panel; Kaggle: attach dataset)")

TRAIN_PATH      = find_input("sft_reflex_418.jsonl")
HELDOUT_PATH    = find_input("heldout_v1.3.jsonl")
print("train  =", TRAIN_PATH)
print("heldout=", HELDOUT_PATH)
CKPT_DIR        = WORK / "checkpoints"
ADAPTER_DIR     = WORK / "adapter_E8_sft_418"
ADAPTER_ZIP     = WORK / "adapter_E8_sft_418.zip"
...
def safe_download(path):
    try:
        ...
        pass  # not Colab, or download unsupported вЂ” disk persists there
...
# Token suppression (01_LAB В§4): Qwen artifact + corpus contamination
SUPPRESS_STRINGS = ["<|file_sep|>", "е§”з»„з»‡йѓЁ"]
...
assert n_train == 418, f"STOP: sft_reflex_418.jsonl = {n_train} lines, expected 418 (E7 artifact lost)"
...
# Prompt format: plain base model (no chat template вЂ” arm C must hold reflex WITHOUT prompt scaffold).
...
# (defensive вЂ” they should not be present in curated reflex pairs; logged if found).
...
# steps/epoch = ceil(418 / (per_device_bs * grad_accum * n_gpu_effective))
...
run_meta = {
    "experiment": "E7_targeted_guard_density (recipe=E2 verbatim lr=1e-4 all-linear; data 323->418: +40 P6_premise_guard +17 P6_disclaim_guard +12 P6_anchor; closes E6 invented-fab seams 20/24/33 source-endorsement + 34 disclaim-then-emit; falsifier: bait_fab(C)=0/n=32 at composite(C)>=5.5 -> GO else E8/DPO)",
...
# GOLD kernel system prompt (arm B / D) вЂ” compact R1-R18, from 03_GOLD_IDENTITY В§3.
GOLD_KERNEL_PROMPT = (
...
#   (2) 7-gram overlap rate: |C 7-grams в€© train 7-grams| / |C 7-grams| > 0.15
# Source corpus for comparison = all 418 train OUTPUT texts.
...
leak_lines = ["# Leak Report вЂ” Arm C vs Train (sft_reflex_418)\n",
...
for item in outputs["C"]:
    ...
    flag = "рџљЁ LEAK" if leaked else "ok"
    leak_lines.append(f"## {item['id']} вЂ” {flag}")
...
leak_lines.insert(3, f"\n**VERDICT: {'LEAK DETECTED вЂ” investigate before trusting C composite' if any_leak else 'NO LEAK вЂ” C outputs are not verbatim recall'}**\n")

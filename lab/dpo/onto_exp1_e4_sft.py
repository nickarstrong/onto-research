# %% [markdown]
# # ONTO · Experiment #1 · E4 — SFT-only Reflex LoRA + P5b guard + core rebalance (Qwen2.5-Coder-7B base)
#
# **Scope (E4 run):** RECIPE = E2 VERBATIM (lr1e-4, all-linear) — UNCHANGED from E3. ONLY data changed: `sft_reflex_323.jsonl` (295 + 20 P5b_no_id_from_recall + 8 P3b_confident_known), run leak-gate,
# emit A/B/C(/D) `outputs.json` for `eval/onto_eval.py` (S3).
#
...
#   - sft_reflex_295.jsonl   (295 rows: instruction + output + category; incl. 59 P5_refusal_on_unknown)
...
TRAIN_PATH      = find_input("sft_reflex_323.jsonl")
HELDOUT_PATH    = find_input("heldout_v1.3.jsonl")
print("train  =", TRAIN_PATH)
print("heldout=", HELDOUT_PATH)
CKPT_DIR        = WORK / "checkpoints"
ADAPTER_DIR     = WORK / "adapter_E4_reflex_lora"
ADAPTER_ZIP     = WORK / "adapter_E4_reflex_lora.zip"
...
assert n_train == 323, f"STOP: sft_reflex_323.jsonl = {n_train} lines, expected 323 (E4 artifact lost)"
...
# steps/epoch = ceil(236 / (per_device_bs * grad_accum * n_gpu_effective))
...
run_meta = {
    "experiment": "E4_guard_rebalance (recipe=E2 verbatim lr=1e-4 all-linear; data 295->323: +20 P5b_no_id_from_recall +8 P3b_confident_known; falsifier: bait_fab(C)=0/n>=30 at composite(C)>=5.5 -> GO else E5 DPO on DOI-seam)",
...
# Source corpus for comparison = all 236 train OUTPUT texts.
...
leak_lines = ["# Leak Report — Arm C vs Train (sft_reflex_295)\n",

...
- pre-register   : PRE_REGISTER_E36.md (FROZEN md5 2e787aebcedfc6e1d4322c44fbb37924) -- sec1/2/4/5/6 operative ; three-way reject/accept inherits PRE_REGISTER_E35 (8c719b0bdc88250165bce434c9652eea)
- lever          : THREE-WAY (FROZEN predicate) -- REJECT (con>=C AND B>=B_floor) RESCUED by ACCEPT (ent>=A AND B>=B_floor); B SHARPENED: B_anchor = containment over {number-like, quoted-span content, CamelCase/ACRONYM entity} ; B=(1-W)*B_lex+W*B_anchor ; W=0 == E35 lexical (regression) ; W swept over W_GRID; premise=source.FINDING / hyp=claim ; MODEL=MoritzLaurer/DeBERTa-v3-large-mnli-fever-anli-ling-wanli (UNCHANGED)
...
- binding axis   : W swept over FROZEN W_GRID [0.0, 0.25, 0.5, 0.75, 1.0] ; B=(1-W)*B_lexical+W*B_anchor ; W=0 = lexical regression anchor (== E35, op MUST reproduce 0.0667)
...
- chosen A (accept) = 0.05 | chosen C = 0.9077617526054382 | chosen B_floor = 0.0 | chosen W (anchor-mix) = 0.0 | B2 at op-point = 0.9
...
- bf-band non-triviality (verdict input, at chosen W=0.0): 2 B_floor points clear GATE_FA -> nontrivial=False (bar >= 3)
- bf-band best-over-W (DIAGNOSTIC, feeds KNIFE_EDGE-vs-BINDING_EXHAUSTED fork): max clears=2 -> best_nontrivial=False

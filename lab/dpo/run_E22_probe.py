...
# E22 = E21 harness, SAME E20-B gate, ONLY the scorer changes:
#   E21 lever = cross-encoder/ms-marco-MiniLM-L-6-v2 (passage RELEVANCE = a topicality signal) -> FAIL_NO_MOVEMENT.
#   E22 lever = model-scale NLI cross-encoder. Score = P(entailment) with premise=source, hypothesis=claim
#               = a claim-SUPPORT signal, not topicality. Single new variable vs the NLI line (E18/E19 small-NLI
#               NO-GO): SCALE. This is the cheap, no-train, frozen-substrate probe of the substrate-vs-architecture
#               fork (NORTH STAR: the reject organ reads FROZEN representations; it is not trained here).
...
def main():
    ...
    ap.add_argument("--out", default="reports/report_E22.md")
    a = ap.parse_args()
    ...
    L = [
        ...
        "",
        f"- lever          : {CE_MODEL} -- P(entailment), premise=source / hypothesis=claim (claim-SUPPORT, not topicality)",
        f"- vs E21         : E21 used ms-marco relevance (topicality) -> FAIL_NO_MOVEMENT. E22 single new var vs NLI line = SCALE.",
        ...
        "PASS -> model-scale entailment SEPARATES support from topicality where small-NLI (E18/E19) could not "
        "-> substrate-floor WAS the bottleneck -> PRE_REGISTER model-scale gate-2 regime (real GPU + model-scale data) | "
        "FAIL_NO_MOVEMENT / FAIL_PARTIAL_MOVEMENT -> even a model-scale entailment representation cannot separate at "
        "iso-recall -> reject-organ failure is ARCHITECTURAL, not scale -> NORTH STAR reframe (ACCEPT/REJECT as separate "
        "organs; non-retrieval reject primitive) | FAIL_RECALL_UNUSABLE -> support-gate destroys gold recall | "
        "VOID_* -> rebuild before any conclusion.",
    ]

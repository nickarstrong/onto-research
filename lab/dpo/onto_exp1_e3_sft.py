# %% [markdown]
# # ONTO · Experiment #1 · E3 — SFT-only Reflex LoRA + P5 refusal (Qwen2.5-Coder-7B base)
#
# **Scope (E3 run):** RECIPE = E2 VERBATIM (lr1e-4, all-linear). ONLY data changed: `sft_reflex_295.jsonl` (236 + 59 P5_refusal_on_unknown), run leak-gate,
...
TRAIN_PATH      = find_input("sft_reflex_295.jsonl")
HELDOUT_PATH    = find_input("heldout_v1.3.jsonl")
print("train  =", TRAIN_PATH)
print("heldout=", HELDOUT_PATH)
CKPT_DIR        = WORK / "checkpoints"
ADAPTER_DIR     = WORK / "adapter_E3_reflex_lora"
ADAPTER_ZIP     = WORK / "adapter_E3_reflex_lora.zip"
...
assert n_train == 295, f"STOP: sft_reflex_295.jsonl = {n_train} lines, expected 295 (E3 artifact lost)"
...
run_meta = {
    "experiment": "E3_refusal (recipe=E2 verbatim lr=1e-4 all-linear; data 236->295: +59 P5_refusal_on_unknown; falsifier: bait_fab(C)<=10% at composite>=5.5)",
...
questions = [(r["id"], r["question"]) for r in held_rows]

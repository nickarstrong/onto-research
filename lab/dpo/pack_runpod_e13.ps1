# pack_runpod_e13.ps1 - E13-shaped stage script (recipe_E13 = decode-time, NOT DPO-train).
# pack_runpod.ps1 throws on recipe_E13 (needs dpo_pairs/lr/beta); this is its E13 sibling.
# Usage: .\pack_runpod_e13.ps1
# Stages ONE upload zip for the E13 GPU run (steps 2-6). ASCII-only + UTF-8 BOM.
$ErrorActionPreference = "Stop"

$dpo   = "C:\Projects\onto-research\lab\dpo"
$stage = "C:\Users\Arist\Downloads\e13_stage"
$zip   = "C:\Users\Arist\Downloads\E13_RUNPOD.zip"
...
$scriptsPublic = @(
  ...
  "onto_e5_gen.py", "recipe_E13.yaml"
)
...
# --- run_e13.sh (pod entrypoint; offline gates first, then driver) ---
...
python gate_pairs.py --heldout data/heldout_v1.3.jsonl --reflex data/e13_provoke_fresh.jsonl --out _local/gate_E13.log
echo 'REVIEW _local/gate_E13.log: pass --gate-passed below ONLY if collision-clean'
echo '=== E13 RUN (steps 2-6); outputs to ./_local ==='
python onto_e13_run.py \
  ...
  --out outputs_E13.json \
  --alphas 0.5,1.0,2.0,4.0 \
  --layers 12,16,24 \
  --gate-passed
echo '=== PACK LOCAL DELIVERABLES (NEVER git) ==='
cd /content/working && zip -rq E13_LOCAL_deliverables.zip _local
cp E13_LOCAL_deliverables.zip /workspace/
echo 'DONE: /workspace/E13_LOCAL_deliverables.zip - Download NOW (LOCAL-ONLY)'
"@
$sh = $sh.Replace("ADAPTERNAME", $adapterName).Replace("`r`n", "`n")
[IO.File]::WriteAllText("$stage\run_e13.sh", $sh)
...
Write-Output "!mkdir -p /content/working && cd /content/working && unzip -o /workspace/E13_RUNPOD.zip -d /content/working && pip install -q -U datasets transformers accelerate peft scikit-learn ``bitsandbytes>=0.46.1``"
Write-Output "ON POD (cell 2 - run):"
Write-Output "!cd /content/working && bash run_e13.sh"
